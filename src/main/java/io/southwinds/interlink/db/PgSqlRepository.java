...
import io.southwinds.interlink.Lib;
import io.southwinds.interlink.Mailer;
import io.southwinds.interlink.conf.Config;
import io.southwinds.interlink.data.*;
import io.southwinds.interlink.event.MQTTEventManager;
import io.southwinds.interlink.security.Jwt;
import io.southwinds.interlink.security.PwdBasedEncryptor;
...
public class PgSqlRepository implements DbRepository {
    ...
    public synchronized Result createOrUpdateUser(String key, UserData user, boolean notifyUser, String[] role) {
        ...
        try {
            ...
            db.setString(8, user.getAcl()); // acl_param
    ...
    private boolean invalidPwd(UserData user, Result result, String newPwd) {
    ...
    public synchronized Result requestPwdReset(String email) {
        ...
        String token = jwt.createJWT(UUID.randomUUID().toString(), "interlink", email, cfg.getSmtpPwdResetTokenExpirySecs() * 1000);
        try {
            mailer.sendResetPwdEmail(email, String.format("Interlink Password Reset"), user.getName(), token);
